...
function pesquisarFilmes() {
    const searchTerm = inputPesquisa.value.trim(); // Obtem o valor do campo de pesquisa e remove espaços em branco no início e no final
    ...
    if (searchTerm !== '') {
        fetch(`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&language=pt-BR&query=${searchTerm}&page=1&include_adult=false`)
            ...
            .catch(error => {
                console.error('Ocorreu um erro ao pesquisar seu filme:', error);
            });
    } else {
